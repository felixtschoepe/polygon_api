...
POLYGON_API_KEY = '63g5kApL_17IUs73a4FWukvDBdlZzJCi'
#ALPACA_API_KEY = os.environ.get('ALPACA_API_KEY')
START_DATE = '2020-08-04'
END_DATE = '2022-08-04'
#TICKERS = 'V'
# URL for all the tickers on Polygon
#POLYGON_TICKERS_URL = 'https://api.polygon.io/v3/reference/tickers?page={}&apiKey={}'
POLYGON_TICKERS_URL = 'https://api.polygon.io/v3/reference/tickers?type=CS&market=stocks&active=true&sort=ticker&order=asc&apiKey=63g5kApL_17IUs73a4FWukvDBdlZzJCi'
...
#                   https://api.polygon.io/v2/aggs/ticker/AAPL/range/1/day/2021-07-22/2021-07-22?adjusted=true&sort=asc&limit=120&apiKey=63g5kApL_17IUs73a4FWukvDBdlZzJCi
# URL FOR DIVIDEND DATA
POLYGON_DIV_URL = 'https://api.polygon.io/v3/reference/dividends?ticker={}&apiKey={}'
#                  https://api.polygon.io/v3/reference/dividends?ticker=AAPL&apiKey=63g5kApL_17IUs73a4FWukvDBdlZzJCi
# URL FOR STOCK SPLITS
POLYGON_SPLIT_URL = 'https://api.polygon.io/v3/reference/splits/{}?apiKey={}'
#URL FOR TICKER TYPES
POLYGON_TYPES_URL = 'https://api.polygon.io/v2/reference/types?apiKey={}'

# #%%
# # Tickerliste erstellen
# TICKERS = {"Symbole": ["AAPL","AMGN","BTI","CSCO","DIS","JNJ","KO","MAIN","MCD","MMM","MO","MSFT","NFLX","NKE","O"
#                     "OHI","PEP","PG","SBUX","SKT","T","TXN","UL","V"]}
# TICKERS_df = pd.DataFrame(TICKERS)
# symbole = TICKERS_df.Symbole.tolist()
# TICKERS_df.to_csv("ticker.csv", index=False, sep=",")
# ##### läuft noch nicht ! ######


# #%%
# # Testen der Datenabholung für Aktie
# # def get_data(TICKERS, outdir, START_DATE, END_DATE):
# #     # Öffne die Session
...
# #     datenlog = open("datenlog.txt", "w")

# #     r = session.get(POLYGON_AGGS_URL.format(TICKERS, START_DATE, END_DATE, POLYGON_API_KEY))
# #     if r:
# #         data = r.json()

# #         # erstelle ein pandas dataframe aus den Informationen
...
# #             df.to_csv('{}/{}.csv'.format(outdir, TICKERS), index=True)
# #             count += 1

# #             # Logging, I could write a short method for this to reuse
# #             msg = (TICKERS + ' Datei erstellt mit Zähler ' + str(data['queryCount']))
# #             print(msg)
# #             datenlog.write(msg)
# #             datenlog.write("\n")

# #         else:
# #             msg = ('Keine Daten für Ticker: ' + str(TICKERS))
# #             print(msg)
# #             datenlog.write(msg)
# #             datenlog.write("\n")
    
# #     datenlog.close()
# #     return ('{} Dateien wurden erstellt'.format(count))
# # '    
# #%% Hole alle Daten zu den Aktien in TICKERS
# get_data(TICKERS, 'data/daten', START_DATE, END_DATE)

# #%%
# # Define a function to pull in the splits data
# # def get_divs(ticker, outdir):

# #     session = requests.Session()
# #     countx = 0
    
# #     # Get the split data
# #     for symbol in ticker: # ['AAPL']:
# #  #       r = session.get(POLYGON_DIV_URL.format(ticker, POLYGON_API_KEY))
# #         data = r.json()
# #         if data['status'] == "OK":
# #             df = pd.DataFrame(data['results'])
# #             # df.rename(columns={'paymentDate': 'date'}, inplace=True)
# #             #df.rename(columns={'ex_dividende_date': 'exdate', 'cash_amount': 'dividend',
# #             #                   'pay_date': 'divPayDate',
# #             #                   'record_date': 'divRecordDate',
# #             #                   'declarartion_ate': 'divDeclaredDate'}, inplace=True)
# #             df.set_index('ex_dividend_date', inplace=True)
# #             df = df.groupby(df.index).first()
# #             df.to_csv('{}/{}.csv'.format(outdir, symbol), index=True)
            
# #             print('div file for ' + symbol + ' ' + str(data['status']))
# #             countx += 1
            
# #     return ('{} file were exported'.format(countx))

# #%%  Pull in all the dividend data
# #get_divs(TICKERS, 'data/divs')
...
# get_bars(TICKERS, 'data/bars', START_DATE, END_DATE)
